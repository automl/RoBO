...
kernels = [(opsign, opname, kernel1name, kernel2name, kernel1, kernel2, kernelop, axes[kernelid]) for kernelid, (kernel1, kernel2, kernel1name, kernel2name) in enumerate(kernelpairs) for kernelop, opsign, opname, axes in [
...
objectives = [(objective1, context_fkt, 'bran', 'product of two Branin function', objective1_min, objective1_S_lower, objective1_S_upper, objective1_dims_Z, objective1_dims_S, f1),
              (objective2, context_fkt, 'hart', 'Hartmann 6 function', objective2_min, objective2_S_lower, objective2_S_upper, objective2_dims_Z, objective2_dims_S, f2)]


for objective, context_fkt, name_short, obj_name, objective_min, S_lower, S_upper, dims_Z, dims_S, fig in objectives:
    for opsign, opname, kernel1name, kernel2name, kernel1, kernel2, kernelop, ax in kernels:
        ...
        plt1, = ax.plot(regret, label='Regret')
        plt2, = ax.plot(contextual_regret, label='Contextual Regret')
        ax.legend(handles=(plt1, plt2))
    #fig.suptitle('Regret of %s' % obj_name)
    fig.tight_layout()
    fig.savefig('%s.svg' % name_short)
