...
model = bn.BaggedNets(robo_net.SGDNet, num_models = 16, bootstrap_with_replacement=False,
					# the remaining arguments are passed to the init of the base model
...
for i in range(len(model.models)):
	plt.figure()
	plt.scatter(X[model.bootstrap_indices[i],0], Y[model.bootstrap_indices[i],0])
	plt.plot(x, model.models[i].predict(x)[:,0])
"""
