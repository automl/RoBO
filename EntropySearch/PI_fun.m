function fx = PI_fun(GP,xmin,xmax,invertsign)
% returns a function, which returns the probability of improvement and its derivative,
% with respect to x.
%
% Philipp Hennig, October 2011
...
fx    = @(x) PI_f(x,alpha,GP,fmin,xmin,xmax,invertsign);
end

function [f,df] = PI_f(x,alpha,GP,fm,xmin,xmax,invertsign)
...
f  = sign * (Phi); % + 1.0e-3); % add tiny probability to smooth out result
df = sign * (-phi/s) * (dmdx + dsdx * z); %  * phi
