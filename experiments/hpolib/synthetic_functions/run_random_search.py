...
num_iterations = 200
output_path = "./experiments/hpolib2/synthetic_functions/results/%s/random_search_%d" % (benchmark, run_id)

os.makedirs(output_path, exist_ok=True)

info = f.get_meta_information()
bounds = np.array(info['bounds'])
res = random_search(f, bounds[:, 0], bounds[:, 1], num_iterations=num_iterations, rng=rng, output_path=None)
