...
num_iterations = 200
output_path = "./experiments/hpolib2/synthetic_functions/results/%s/rf_log_ei_%d" % (benchmark, run_id)

os.makedirs(output_path, exist_ok=True)

info = f.get_meta_information()
bounds = np.array(info['bounds'])
res = bayesian_optimization(f, bounds[:, 0], bounds[:, 1],
                            num_iterations=num_iterations, n_init=2,
                            rng=rng, output_path=None, model_type="rf")
