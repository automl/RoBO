...
def main(names, dirs):
    ...
    for i in range(n_curves):
        ...
        #ax1.fill_between(iterations, mean + 3 * np.sqrt(var), mean - np.sqrt(var), facecolor=color[i % len(color)], alpha=0.2)
        ...
        ax2.fill_between(iterations, mean + 3 * np.sqrt(var), mean - np.sqrt(var), facecolor=color[i % len(color)], alpha=0.2)
