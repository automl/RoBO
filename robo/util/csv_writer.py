...
def csv_reader(filepath):
    with open(filepath, 'rb') as csvfile: #opens the csv file
        result = {}
        reader = csv.DictReader(csvfile, delimiter = ',') # reads from the DICT Reader with that openend file
        for row in reader:
...
def result_dict(filepath):
    ...
    for k in filepath:
        ...
        for i in directory_tree[0][1]: #add the first descendandts of the main file path in the dictionary first
            dictionary[i]=" "
