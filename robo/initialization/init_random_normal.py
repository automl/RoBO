...
def init_random_normal(x_lower, x_upper, n, mean=None, std=None):
    ...
    :param mean: mean for each dimension (n array).
                If None, the mean of the range is used.
    :param std: standard deviation for each dimension (n array).
                If None, the width of the range is used.
    ...
    return np.array([np.clip(np.random.normal(mean[i], std[i], n), x_lower[i], x_upper[i])
